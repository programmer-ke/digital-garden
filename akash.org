...
- Take part in governance by voting on proposals to modify network
  params, upgrades to protocol and community funds
...
** Akash Node                                           :blockchain:node:rpc:

Roles:

*** Blockchain synchronization
- Proposes and validates new blocks via consensus
- Uses a gossip protocol for communication
    
*** Transaction Submission
- Validates, adds and propagates transactions to the mempool e.g.
  creating deployments, leases, bids etc
  
*** Querying network state

- Maintains an indexed view of the blockhain to allow querying of
  information such as user balances and deployment status

** Akash Providers                                                :providers:

*** Key components

**** Provider Daemon (akashd)                                 :daemon:akashd:
Communicates to the blockchain and manages the providers resources

**** Container orchestration                                     :kubernetes:
Provision of containers for deploying and scaling user applications

** Deployments

*** Akash CLI deployment                                    :cli:deployments:
...
# Close deployment
provider-services tx deployment close --from $AKASH_KEY_NAME --dseq $AKASH_DSEQ

# execute remote shell
./provider-services lease-shell --from $AKASH_KEY_NAME --dseq $AKASH_DSEQ --provider=$AKASH_PROVIDER <service_name> cat /etc/passwd
#+end_src

# Authorize spending
./provider-services tx authz grant $AKASH_TEST_ACCOUNT_ADDRESS deposit --spend-limit=20000000uakt --from $AKASH_ACCOUNT_ADDRESS --scope deployment

# View authorization
provider-services query authz grants $AKASH_ACCOUNT_ADDRESS $AKASH_TEST_ACCOUNT_ADDRESS


# Authorize feegrant for tx fees
./provider-services tx feegrant grant $AKASH_TEST_ACCOUNT_ADDRESS --spend-limit 10000000uakt --from $AKASH_ACCOUNT_ADDRESS 
...
Attributes can specified on the SDL and only providers that support
those attributes can bid for the deployment.

A signedby spec can be used to only select providers who's attributes
have been audited by the an auditor with the specified keys:
 - Can use =anyOf= or =allOf= to select from multiple auditor keys

Attributes and Signed by keys are each optional, and are specified
in the SDL profile placement section.

** IP Leases

Not all providers support IP leasing. The bid engine automatically
filters them out during the bidding process.

IP leases can be tranfered to new deployment on the same provider.

All valid ports, 1 - 65535 are available for use with IP
leases. Unique ports can be used for different services within a
deployment on the provider.
 
** Persistent Storage

Persists only during the lease.

Shared volumes between services not supported. If a single profile is
used by multiple services, separate storage volumes will be assigned.

If not marked as persistent, it will be cleared during restarts.
