...
*** Metacharacters      
- Special characters that are interpreted by the shell before executing commands
...
- $0 is the name of the command itselt being executed by the subshell
- backtics allow a program's output be used as arguments

*** Shell variables

- Defined for the current shell and not accessible in sub shells unless exported
