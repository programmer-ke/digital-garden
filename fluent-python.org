...
- When declaring a parameterized generic like =Sequence[T]=, =TypeVar=
  ...
  deep changes to the interpretter to support the generic type notion.
...
- =Callable[[], int]= is a subtype of =Callable[[], float]=
  - This is because =int= is a subtype of =float=. =int= supports all
    operations on =float=
- =Callable[[int], None]= is however not a subtype of
  =Callable[[float], None]=, rather its the other way round.
